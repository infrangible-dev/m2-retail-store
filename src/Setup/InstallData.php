    ...
    implements InstallDataInterface
{
    ...
    {
        $this->retailStoreHelper->addAttribute($setup, 'code', 'Retail Store Code', 'varchar', 'text', null, 10, false,
            true);
        $this->retailStoreHelper->addAttribute($setup, 'status', 'Retail Store Status', 'int', 'text', '1', 20, false,
            true);
        $this->retailStoreHelper->addAttribute($setup, 'name', 'Retail Store Name', 'varchar', 'text', null, 30, false,
            true, true, true);
        $this->retailStoreHelper->addAttribute($setup, 'description', 'Retail Store Description', 'text', 'textarea',
            null, 40, false, false, true, true);
        $this->retailStoreHelper->addAttribute($setup, 'direction', 'Retail Store Direction', 'text', 'textarea', null,
            50, false, false, true, true);
        $this->retailStoreHelper->addAttribute($setup, 'email', 'Retail Store E-Mail', 'varchar', 'text', null, 60,
            false, true, true, true);
        $this->retailStoreHelper->addAttribute($setup, 'phone_number', 'Retail Store Phone Number', 'varchar', 'text',
            null, 70, false, false, true, true);
        $this->retailStoreHelper->addAttribute($setup, 'fax_number', 'Retail Store Fax Number', 'varchar', 'text', null,
            80, false, false, true, true);
        $this->retailStoreHelper->addAttribute($setup, 'opening_hours', 'Retail Store Opening Hours', 'text',
            'textarea', null, 90, false, false, true, true);
        $this->retailStoreHelper->addAttribute($setup, 'opening_hours_special', 'Retail Store Special Opening Hours',
            'text', 'textarea', null, 100, false, false, true, true);
        $this->retailStoreHelper->addAttribute($setup, 'image', 'Retail Store Image', 'text', 'text', null, 110, false,
            false, true, true);
        $this->retailStoreHelper->addAttribute($setup, 'street', 'Retail Store Street', 'varchar', 'text', null, 120,
            false, true, true, true);
        $this->retailStoreHelper->addAttribute($setup, 'postcode', 'Retail Store Postcode', 'varchar', 'text', null,
            130, false, true, true, true);
        $this->retailStoreHelper->addAttribute($setup, 'city', 'Retail Store City', 'varchar', 'text', null, 140, false,
            true, true, true);
        $this->retailStoreHelper->addAttribute($setup, 'country', 'Retail Store Country', 'varchar', 'select', null,
            150, false, true, true, true, null, Country::class);
        $this->retailStoreHelper->addAttribute($setup, 'latitude', 'Retail Store Latitude', 'decimal', 'text', null,
            160, false, true);
        $this->retailStoreHelper->addAttribute($setup, 'longitude', 'Retail Store Longitude', 'decimal', 'text', null,
            170, false, true);
        $this->retailStoreHelper->addAttribute($setup, 'map_image', 'Retail Store Map Image', 'text', 'textarea', null,
            180, false, false, true, true);
        $this->retailStoreHelper->addAttribute($setup, 'feature', 'Retail Store Feature', 'text', 'multiselect', null,
            190, false, true, true, true, ArrayBackend::class, Feature::class);
        $this->retailStoreHelper->addAttribute($setup, 'click_and_collect_type',
